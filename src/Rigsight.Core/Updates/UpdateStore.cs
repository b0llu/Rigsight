...
public static class UpdateStore
{
    ...
    /// <summary>Deletes installers other than <paramref name="keep"/>'s (older ones, and all of them once up to date).</summary>
    public static void Tidy(Release? keep)
    {
        try
        {
            ...
            foreach (var file in Directory.EnumerateFiles(Folder, "Rigsight-Setup-*"))
                if (!string.Equals(file, keepPath, StringComparison.OrdinalIgnoreCase)) File.Delete(file);
        }
        catch { }
    }
    ...
    /// what GitHub published (<see cref="InvalidDataException"/> otherwise). Progress is reported in bytes, about once per percent.
    /// </summary>
    public static async Task DownloadAsync(Release release, IProgress<long>? progress = null, CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            await using (var output = new FileStream(part, FileMode.Create, FileAccess.Write, FileShare.None, 1 << 16, useAsync: true))
            {
                ...
                long done = 0, step = Math.Max(release.Size / 100, 1), next = 0;
                int read;
                while ((read = await input.ReadAsync(buffer, ct)) > 0)
                {
                    ...
                    if (done >= next) { progress?.Report(done); next = done + step; }
                }
